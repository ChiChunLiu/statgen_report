...
c. 
When performing association analysis, we can use mixed effect models with a genetic relationship matrix to correct for cryptic relatedness and population structure. We can also intergrate large-scale pedigree into our relationship matrix if the information is available.
...
b. 
We will make use of transcriptome studies from publicly available databases such as GTEx. Also, we can run `Predixcan` on multiple tissue types and run enrichment analyses on the results.

c. 
We will again utilize utilize public available resources such as ENCODE as annotation. After computationally predicting effects of the variants we can look into experimental databases of model organisms.
